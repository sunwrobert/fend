...
function updatePositions() {
  ...
  var sinScroll = scrollTop / 1250;
  var phases = [Math.sin(sinScroll), Math.sin(sinScroll + 1), Math.sin(sinScroll + 2), Math.sin(sinScroll + 3), Math.sin(sinScroll + 4)];
  var iMod5 = 0;
  for (var i = 0; i < items.length; i++) {
    var phase = phases[iMod5++];
    iMod5 = iMod5 > 4 ? 0 : iMod5;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  for (var i = 0; i < 200; i++) {
    ...
    elem.basicLeft = iModCols++ * s;
    iModCols = iModCols > 7 ? 0 : iModCols;
